...
class whiteboard_print_api
{
   public function printWhiteboardMenu ()
   {
      ...
      {
         $version_management_access_level = $whiteboard_config_api->whitebaord_plugin_config_get ( 'version_management_access_level', 'VersionManagement' );
         if ( ( user_get_access_level ( $user_id, $project_id ) >= $version_management_access_level ) || user_is_administrator ( $user_id ) )
         {
            echo '<td>';
            echo '| ';
            echo '<a href="' . plugin_page ( 'version_view', false, 'VersionManagement' ) . '&amp;edit=0">' . plugin_lang_get ( 'menu_title', 'VersionManagement' ) . '</a>';
            echo '</td>';
         }
      }
